...
import { useBranch, useRoles } from "../hook/useUser";
import { type CreateUsers } from "../user-types";
...
export const NewUsuarios = ({ openF, handleCloseF, userData }: NewUsuariosProps) => {
    const { register, control, handleSubmit, formState: { errors }, reset, setValue } = useForm<CreateUsers>({
    ...
    const onSubmit = (data: CreateUsers) => {
        if (userData) {
            ...
            console.log("Crear nuevo usuario:", data);
        }
    };
    ...
    useEffect(() => {
        if (openF) {
            ...
            if (userData?.branchId && branch) {
                const selectedBranch = branch.find(branch => branch.id === userData.branchId);
                if (selectedBranch) {
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Autocomplete
                                    ...
                                    value={branch?.find(role => role.id === field.value) || null}
